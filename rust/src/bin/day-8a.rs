...
fn main() -> std::io::Result<()> {
    let grid: Vec<Vec<u32>> = include_str!("../../../input/day-8.txt")
        .split('\n')
        .map(|line| {
            line.chars().map(|x| x as u32 - 0x30).collect()
        })
        .collect();
    // println!("{:?}", grid);

    let row_visibles: Vec<BinLength> = grid.iter()
    ...
    let transposed_columns: Vec<BinLength> = (0..column_visibles.len())
        .map(|i| {
            column_visibles.iter()
                .enumerate()
                .map(|(j, col)| (col >> (column_visibles.len() - 1 - i) & 1) << (column_visibles.len() - 1 - j))
                .sum()
        })
        .collect();

    let visible: u32 = row_visibles.into_iter()
