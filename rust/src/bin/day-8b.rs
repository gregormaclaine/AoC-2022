...
fn main() -> std::io::Result<()> {
    let grid: Vec<u32> = include_str!("../../../input/day-8.txt")
        .split('\n')
        .flat_map(|line| {
            line.chars().map(|x| x as u32 - 0x30).collect::<Vec<u32>>()
        })
        .collect();
